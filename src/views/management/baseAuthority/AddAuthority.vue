        ...
        <a-col :span="11">
          <a-form layout="inline" class="form-flex" :label-col="{ span: 8 }" :wrapper-col="{ span: 16 }">
            ...
            <a-form-item
              ...
              label="路由路径">
              <a-input name="htmlurl" placeholder="请输入路由路径" v-model="queryParam.htmlurl"/>
            </a-form-item>
            <a-form-item
              v-if="queryParam.type==1"
              label="路由key">
              <a-input name="name" placeholder="请输入路由key" v-model="queryParam.namekey"/>
            </a-form-item>
            <a-form-item
              v-if="queryParam.type==1"
              label="路由访问路径">
              <a-input name="name" placeholder="请输入路由访问路径" v-model="queryParam.path"/>
            </a-form-item>
            <a-form-item
              v-if="queryParam.type==1"
              label="路由重定向">
              <a-input name="name" placeholder="请输入路由重定向" v-model="queryParam.redirect"/>
            </a-form-item>
            <a-form-item
              v-if="queryParam.type==1"
              label="路由图标">
              <a-input name="name" placeholder="请输入路由图标" v-model="queryParam.icon"/>
            </a-form-item>
            <a-form-item
              v-if="queryParam.type==1"
              label="是否隐藏">
              <a-radio-group v-model="queryParam.showMenu">
                <a-radio value="1">
                  是
                </a-radio>
                <a-radio value="0">
            ...
            <a-form-item
              v-if="queryParam.type==1"
              label="是否隐藏子菜单">
              <a-radio-group v-model="queryParam.hideChildren">
                <a-radio value="1">
                  是
                </a-radio>
                <a-radio value="0">
